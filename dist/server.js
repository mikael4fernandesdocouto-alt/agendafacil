...
// ============================================================
const app = (0, express_1.default)(); // Cria o app Express
const PORT = process.env.PORT || 3000; // Porta (3000 ou variável de ambiente)
// MIDDLEWARE = funções que rodam ANTES de cada rota
app.use((0, cors_1.default)()); // Permite requisições de outros domínios (front local)
app.use(express_1.default.json()); // Converte o body JSON pra objeto JavaScript automaticamente
app.use(express_1.default.static(path_1.default.join(__dirname, "..", "front"))); // Serve arquivos HTML/CSS/JS da pasta front/
// ============================================================
...
app.post("/api/estabelecimentos", (req, res) => {
    try {
        ...
        (0, database_1.run)(`INSERT INTO estabelecimentos (id, nome, slug, email, senha, telefone, endereco, google_maps_url)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [id, nome, slug, email, senha, telefone || null, endereco || null, google_maps_url || null]);
...
app.get("/api/estabelecimentos/slug/:slug", (req, res) => {
    try {
        ...
        const est = (0, database_1.get)("SELECT id, nome, slug, telefone, endereco, google_maps_url FROM estabelecimentos WHERE slug = ?", [req.params.slug]);
        ...
        const servicos = (0, database_1.all)("SELECT * FROM servicos WHERE estabelecimento_id = ? AND ativo = 1", [est.id]);
        // 4. Busca os horários de funcionamento
        const horarios = (0, database_1.all)("SELECT * FROM horarios_funcionamento WHERE estabelecimento_id = ?", [est.id]);
...
app.post("/api/login", (req, res) => {
    try {
        ...
        const est = (0, database_1.get)("SELECT id, nome, slug FROM estabelecimentos WHERE email = ? AND senha = ?", [email, senha]);
...
app.get("/api/servicos/:estabelecimento_id", (req, res) => {
    try {
        const servicos = (0, database_1.all)("SELECT * FROM servicos WHERE estabelecimento_id = ? AND ativo = 1", [req.params.estabelecimento_id]);
        res.json(servicos);
    }
    catch (err) {
...
app.post("/api/servicos", (req, res) => {
    try {
        ...
        const id = (0, uuid_1.v4)();
        (0, database_1.run)("INSERT INTO servicos (id, estabelecimento_id, nome, duracao_minutos, preco) VALUES (?, ?, ?, ?, ?)", [id, estabelecimento_id, nome, duracao_minutos, preco || 0]);
...
app.delete("/api/servicos/:id", (req, res) => {
    try {
        (0, database_1.run)("UPDATE servicos SET ativo = 0 WHERE id = ?", [req.params.id]);
...
app.get("/api/agendamentos/:estabelecimento_id/:data", (req, res) => {
    try {
        const agendamentos = (0, database_1.all)(`SELECT a.*, s.nome as servico_nome, s.duracao_minutos
       FROM agendamentos a
       JOIN servicos s ON a.servico_id = s.id
       WHERE a.estabelecimento_id = ? AND a.data = ? AND a.status = 'agendado'
       ORDER BY a.hora`, [req.params.estabelecimento_id, req.params.data]);
...
app.post("/api/agendamentos", (req, res) => {
    try {
        ...
        const ocupado = (0, database_1.get)(`SELECT COUNT(*) as total FROM agendamentos
       WHERE estabelecimento_id = ? AND data = ? AND hora = ? AND status = 'agendado'`, [estabelecimento_id, data, hora]);
        ...
        const id = (0, uuid_1.v4)();
        (0, database_1.run)(`INSERT INTO agendamentos (id, estabelecimento_id, servico_id, nome_paciente, telefone_paciente, data, hora)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [id, estabelecimento_id, servico_id, nome_paciente, telefone_paciente, data, hora]);
        res.status(201).json({
            id,
...
app.patch("/api/agendamentos/:id/cancelar", (req, res) => {
    try {
        (0, database_1.run)("UPDATE agendamentos SET status = 'cancelado' WHERE id = ?", [req.params.id]);
...
app.get("/api/dashboard/:estabelecimento_id", (req, res) => {
    ...
    `;
        ...
        const inicioStr = Array.isArray(inicio) ? inicio[0] : inicio;
        const fimStr = Array.isArray(fim) ? fim[0] : fim;
        ...
        const agendamentos = (0, database_1.all)(sql, params);
        res.json(agendamentos);
    }
    catch (err) {
